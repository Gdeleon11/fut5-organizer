...
export function confirmedPlayersForMatch(match, attendances = [], profiles = []) {
  ...
  return attendances
    .filter((attendance) => attendance.match_id === match.id)
...
export function generateReservationText({ match, attendances = [], profiles = [] }) {
  const responsible = profiles.find((profile) => profile.id === match.reservation_owner_user_id);
  ...
  const dateText = (match.preferred_dates || []).length
